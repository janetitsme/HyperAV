...
<?php 
	...
	if ($results) {
		if ($num_rows > 0) {
			...
			echo '<table border="2">
			<tr><td></td><th><b>Customer Id</b></th><th>First Name</th><th>Last Name</th><th>Address1</th><th>Address2</th><th>PostCode</th><th>Telephone</th><th>Email</th><th></th></tr>';

			while ($row = mysqli_fetch_array($results, MYSQLI_ASSOC)) {
				echo '<tr><td>' .$row['customerID']. '</td><td>' . $row['cuFName']. ' </td><td>' . $row['cuLName'] . '</td><td>' . $row['cuAddress1'] . '</td><td>' . $row['cuAddress2'] . '</td><td>' . $row['cuAddress2'] . '</td><td>' .$row['cuPostcode'] . '</td><td>' . $row['cuTelephone'] . '</td><td>' . $row['cuEmail'] . '</td><td>
