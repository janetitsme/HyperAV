...
<?php 
	...
	if($results1) {
		...
		<form action="products.php" method="GET">
			...
			<?php } ?>
			</select><noscript><INPUT type="submit" value="Select" name=category></noscript> 
		</form><?php
		}
	}
?>
...
<?php 
	...
	if(isset($_GET['category'])) {
		$query = 'SELECT * FROM hyperav_products WHERE prCategory = "' . $_GET['category'] . '"';
	} else {
		...
		if ($num_rows > 0) {
			...
			echo'<table> 			
			 ...
			 while ($row = mysqli_fetch_array($results, MYSQLI_ASSOC)) {
				echo '<p> <tr><td><img src="images/' . $row['prName'] . '.jpg" id="product_images"></td><td width="100px"><a href="selected_product.php?prModelNo=' . $row['prModelNo'] . '">' . $row['prName'] . '</a></td><td width="250px">' . $row['prDescription'] . '</td><td width="100px">&pound' . $row['prPrice'] . '</td><td width="100px">' . $row['prCategory'] . '</td><td>
